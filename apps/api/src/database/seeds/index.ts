...
const AppDataSource = new DataSource({
  type: 'postgres',
  url: process.env.DATABASE_URL || 'postgresql://simplenow:simplenow@localhost:5432/simplenow',
  entities: [User, Ticket, Article, ConfigurationItem, Workflow, CatalogItem],
  synchronize: true,
});
...
async function seed() {
  ...
  const ticketData = [
    // Critical Incidents
    { number: 'INC-0001', type: TicketType.INCIDENT, title: 'Production database cluster is down - all services affected', description: 'The primary database cluster has failed. All production services are experiencing downtime. Approximately 500 users affected. The cluster became unresponsive at 09:42 UTC. Monitoring alerts triggered for all dependent services.', status: TicketStatus.IN_PROGRESS, priority: TicketPriority.CRITICAL, category: 'Infrastructure', affectedService: 'Database', assigneeId: users[1].id, reporterId: users[0].id, aiSentiment: 'urgent', aiSentimentScore: 0.95, aiSummary: 'Critical database outage affecting all production services. Immediate escalation required.', tags: ['database', 'outage', 'production', 'critical'], slaBreachAt: new Date(now.getTime() + 30 * 60000) },
    { number: 'INC-0002', type: TicketType.INCIDENT, title: 'VPN service degraded - remote workers cannot connect', description: 'Multiple remote workers are reporting inability to connect to the corporate VPN. Issue started around 8:30 AM. Approximately 120 remote employees affected. Teams chat shows widespread complaints.', status: TicketStatus.OPEN, priority: TicketPriority.HIGH, category: 'Network', affectedService: 'VPN', assigneeId: users[2].id, reporterId: users[0].id, aiSentiment: 'frustrated', aiSentimentScore: 0.7, aiSummary: 'VPN degradation affecting remote workforce access.', tags: ['vpn', 'network', 'remote-access'], slaBreachAt: new Date(now.getTime() + 2 * 3600000) },
    { number: 'INC-0003', type: TicketType.INCIDENT, title: 'Email server not delivering messages for the past 2 hours', description: 'Users are reporting that emails are not being delivered. Sent emails show as "sent" in client but recipients are not receiving them. Issue affects both internal and external communications.', status: TicketStatus.IN_PROGRESS, priority: TicketPriority.HIGH, category: 'Email & Collaboration', affectedService: 'Email', assigneeId: users[1].id, reporterId: users[4].id, aiSentiment: 'frustrated', aiSentimentScore: 0.65, tags: ['email', 'communication'], slaBreachAt: new Date(now.getTime() + 1.5 * 3600000) },
    { number: 'INC-0004', type: TicketType.INCIDENT, title: 'Cannot login to Salesforce CRM - getting 503 error', description: 'Our Salesforce org is returning 503 errors for all users. The sales team cannot access customer data or log calls. Revenue impact estimated at $50k/hour.', status: TicketStatus.OPEN, priority: TicketPriority.CRITICAL, category: 'Application', affectedService: 'Salesforce CRM', reporterId: users[4].id, aiSentiment: 'urgent', aiSentimentScore: 0.9, tags: ['salesforce', 'crm', 'access'], slaBreachAt: new Date(now.getTime() + 45 * 60000) },
    { number: 'INC-0005', type: TicketType.INCIDENT, title: 'WiFi intermittently dropping in Building A conference rooms', description: 'Conference rooms 3A, 3B, and 4C are experiencing intermittent WiFi drops. Issue is disrupting video calls and presentations. Approximately 20 people affected.', status: TicketStatus.OPEN, priority: TicketPriority.MEDIUM, category: 'Network', affectedService: 'WiFi', assigneeId: users[2].id, reporterId: users[4].id, aiSentiment: 'neutral', aiSentimentScore: 0.3, tags: ['wifi', 'network', 'building-a'], slaBreachAt: new Date(now.getTime() + 6 * 3600000) },
    // Medium/Low incidents
    { number: 'INC-0006', type: TicketType.INCIDENT, title: "User's laptop won't turn on after Windows update", description: "John Smith's laptop (Asset: LAP-2847) failed to boot after installing Windows update KB5034441. Shows blue screen error 0x800F0922.", status: TicketStatus.RESOLVED, priority: TicketPriority.MEDIUM, category: 'Hardware', affectedService: 'Endpoint', assigneeId: users[1].id, reporterId: users[0].id, resolvedAt: new Date(now.getTime() - 2 * 3600000), tags: ['laptop', 'windows-update', 'hardware'] },
    { number: 'INC-0007', type: TicketType.INCIDENT, title: 'Suspicious login attempt detected from foreign IP', description: 'Security monitoring detected 47 failed login attempts from IP 185.220.101.45 (Russia) targeting admin accounts in the last 10 minutes.', status: TicketStatus.IN_PROGRESS, priority: TicketPriority.HIGH, category: 'Security', assigneeId: users[3].id, reporterId: users[0].id, aiSentiment: 'urgent', aiSentimentScore: 0.85, tags: ['security', 'breach-attempt', 'login'] },
    { number: 'INC-0008', type: TicketType.INCIDENT, title: 'Printer in Finance department printing garbled text', description: 'The HP LaserJet in Finance (3rd floor) is printing garbled characters. Started after yesterday\'s network maintenance window.', status: TicketStatus.OPEN, priority: TicketPriority.LOW, category: 'Hardware', tags: ['printer', 'hardware'], slaBreachAt: new Date(now.getTime() + 20 * 3600000) },
    // Service Requests
    { number: 'REQ-0001', type: TicketType.SERVICE_REQUEST, title: 'Request new MacBook Pro for new hire - Marketing team', description: 'New hire Jane Doe starting on Monday needs: MacBook Pro 14" M3, 16GB RAM, 512GB SSD. Please configure with standard marketing tools.', status: TicketStatus.OPEN, priority: TicketPriority.MEDIUM, category: 'Hardware Provisioning', assigneeId: users[1].id, reporterId: users[4].id, tags: ['hardware', 'new-hire', 'macbook'] },
    { number: 'REQ-0002', type: TicketType.SERVICE_REQUEST, title: 'Access request: Adobe Creative Cloud for design team', description: 'The design team (5 members) requires Adobe Creative Cloud licenses. Approved by manager Lisa Chen. Budget code: MKT-2024-CC.', status: TicketStatus.PENDING, priority: TicketPriority.LOW, category: 'Software Licensing', reporterId: users[4].id, tags: ['adobe', 'software', 'license'] },
    { number: 'REQ-0003', type: TicketType.SERVICE_REQUEST, title: 'Set up Microsoft 365 for new employee David Park', description: 'Create M365 account, assign E3 license, add to Engineering distribution lists, set up MFA.', status: TicketStatus.IN_PROGRESS, priority: TicketPriority.MEDIUM, category: 'Access Management', assigneeId: users[1].id, reporterId: users[0].id, tags: ['m365', 'new-hire', 'access'] },
    // Problems
    { number: 'PRB-0001', type: TicketType.PROBLEM, title: 'Recurring VPN disconnections - pattern analysis needed', description: 'Over the past 3 weeks, there have been 12 VPN-related incidents. AI pattern detection suggests common root cause. Linked incidents: INC-0002 and 6 historical incidents.', status: TicketStatus.IN_PROGRESS, priority: TicketPriority.HIGH, category: 'Network', assigneeId: users[2].id, reporterId: users[0].id, aiRootCause: 'Analysis suggests the FortiGate VPN concentrator has a memory leak in firmware version 7.2.4. Recommend upgrading to 7.2.5.', linkedIncidents: ['INC-0002'], tags: ['vpn', 'problem', 'pattern'] },
    { number: 'PRB-0002', type: TicketType.PROBLEM, title: 'Database performance degradation pattern - 8 incidents in 2 weeks', description: 'Multiple database slow-query incidents have been raised. Pattern detected across 8 incidents over 14 days. Need root cause analysis.', status: TicketStatus.OPEN, priority: TicketPriority.HIGH, category: 'Infrastructure', reporterId: users[0].id, linkedIncidents: ['INC-0001'], tags: ['database', 'performance', 'problem'] },
    // Changes
    { number: 'CHG-0001', type: TicketType.CHANGE, title: 'Upgrade PostgreSQL from 14.x to 16.x on production cluster', description: 'Major version upgrade required to address security vulnerabilities and performance improvements. Includes 4-hour maintenance window. Full rollback plan documented. Tested in staging.', status: TicketStatus.PENDING, priority: TicketPriority.HIGH, category: 'Infrastructure', changeType: 'major', changeRisk: 'high' as any, changeRiskScore: 68, assigneeId: users[1].id, reporterId: users[0].id, scheduledStartAt: new Date(now.getTime() + 7 * 24 * 3600000), scheduledEndAt: new Date(now.getTime() + 7 * 24 * 3600000 + 4 * 3600000), tags: ['database', 'upgrade', 'postgresql'] },
    { number: 'CHG-0002', type: TicketType.CHANGE, title: 'Deploy new API gateway configuration for rate limiting', description: 'Add rate limiting rules to prevent API abuse. Configuration tested in staging for 2 weeks. Low risk change.', status: TicketStatus.OPEN, priority: TicketPriority.MEDIUM, category: 'Application', changeType: 'minor', changeRisk: 'low' as any, changeRiskScore: 22, assigneeId: users[2].id, reporterId: users[0].id, tags: ['api', 'rate-limiting', 'security'] },
  ];

  const tickets = await ticketRepo.save(ticketData.map(t => ticketRepo.create({
    ...t,
    worklogs: t.status === TicketStatus.IN_PROGRESS || t.status === TicketStatus.RESOLVED ? [
      { id: '1', authorId: t.assigneeId || users[0].id, authorName: 'Sarah Chen', content: 'Investigating the issue. Initial analysis shows potential network timeout.', timeSpentMinutes: 30, isPublic: true, createdAt: new Date(now.getTime() - 3600000).toISOString() },
    ] : [],
    comments: [],
    attachments: [],
  })));
  ...
  const articles = await articleRepo.save([
    { title: 'How to Reset Your VPN Password', content: '## Problem\nYou\'ve forgotten your VPN password or it has expired.\n\n## Solution\n\n1. Navigate to https://vpn.company.com/reset\n2. Enter your corporate email address\n3. Check your email for the reset link (valid for 15 minutes)\n4. Follow the link to create a new password\n\n### Password Requirements\n- Minimum 12 characters\n- Must include uppercase, lowercase, number, and special character\n\n## Prevention\nEnable the VPN password manager integration to auto-rotate credentials.', summary: 'Step-by-step guide to reset VPN credentials.', category: 'Network', tags: ['vpn', 'password', 'reset', 'howto'], status: ArticleStatus.PUBLISHED, viewCount: 342, helpfulCount: 89 },
    { title: 'Windows Update Failed - Boot Loop Fix', content: '## Symptoms\n- Blue screen after Windows update\n- Error code: 0x800F0922 or 0x80240034\n- System stuck in restart loop\n\n## Solution\n\n### Method 1: Automatic Repair\n1. Boot from Windows installation media\n2. Select "Repair your computer"\n3. Navigate to Troubleshoot > Advanced options > Startup Repair\n\n### Method 2: System Restore\n1. Boot into Safe Mode (F8 during startup)\n2. Open System Restore\n3. Select a restore point before the failed update\n\n## Prevention\nAlways create a system restore point before applying major updates.', summary: 'Fix Windows boot loop caused by failed Windows update.', category: 'Hardware', tags: ['windows', 'update', 'boot-loop', 'bluescreen'], status: ArticleStatus.PUBLISHED, viewCount: 567, helpfulCount: 234 },
    { title: 'Setting Up Multi-Factor Authentication (MFA)', content: '## Overview\nMFA is required for all corporate accounts. This guide walks through the setup process.\n\n## Required App\nDownload **Microsoft Authenticator** from your device\'s app store.\n\n## Setup Steps\n1. Log in to https://myaccount.microsoft.com\n2. Click "Security info" in the left navigation\n3. Click "+ Add sign-in method"\n4. Select "Authenticator app"\n5. Follow the QR code scanning instructions\n6. Complete verification\n\n## Troubleshooting\n- If QR scan fails, use the manual code entry option\n- Time sync issues: ensure your phone time is set to automatic', summary: 'Complete guide for setting up MFA on corporate accounts.', category: 'Security', tags: ['mfa', 'security', 'authentication', '2fa'], status: ArticleStatus.PUBLISHED, viewCount: 1203, helpfulCount: 456 },
    { title: 'New Employee IT Onboarding Checklist', content: '## IT Onboarding Checklist\n\n### Day 1\n- [ ] Laptop/computer provisioned and delivered\n- [ ] Corporate email setup (first.last@company.com)\n- [ ] Microsoft 365 license assigned\n- [ ] MFA configured\n- [ ] VPN access granted\n\n### Week 1\n- [ ] Corporate password policy reviewed\n- [ ] Security awareness training completed\n- [ ] All required software installed\n- [ ] Badge access configured\n\n### Software Standards\n- **Productivity**: Microsoft 365\n- **Video calls**: Microsoft Teams\n- **Password manager**: 1Password\n- **VPN**: FortiClient', summary: 'Complete IT checklist for new employee onboarding.', category: 'Onboarding', tags: ['onboarding', 'new-hire', 'checklist', 'setup'], status: ArticleStatus.PUBLISHED, viewCount: 892, helpfulCount: 312 },
    { title: 'Email Not Sending - Troubleshooting Guide', content: '## Common Causes\n1. Email stuck in Outbox\n2. Attachment too large (>25MB limit)\n3. Recipient server blocking emails\n4. Outlook profile corruption\n\n## Quick Fix Steps\n1. Check if Outlook is in "Work Offline" mode\n2. Try sending from web (outlook.office.com)\n3. Clear the Outbox manually\n4. Rebuild Outlook profile if issues persist\n\n## Large File Alternative\nFor files >25MB, use **SharePoint** or **OneDrive** sharing links instead of attachments.', summary: 'Troubleshoot email delivery issues in Microsoft Outlook.', category: 'Email & Collaboration', tags: ['email', 'outlook', 'troubleshoot'], status: ArticleStatus.PUBLISHED, viewCount: 445, helpfulCount: 178 },
    { title: 'Database Connection Pool Exhaustion - Resolution Guide', content: '## Symptoms\n- Applications returning "Too many connections" errors\n- Database CPU at 100%\n- Response times exceeding 30 seconds\n\n## Immediate Actions\n1. Check active connections: `SELECT count(*) FROM pg_stat_activity;`\n2. Kill idle connections: `SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE state = \'idle\' AND state_change < NOW() - INTERVAL \'5 min\';`\n\n## Root Cause Analysis\nCommon causes include:\n- Connection leaks in application code\n- Missing connection pool configuration\n- Query optimization needed\n\n## Long-term Fix\nImplement PgBouncer as a connection pooler.', summary: 'Resolve database connection pool exhaustion issues.', category: 'Infrastructure', tags: ['database', 'postgresql', 'performance', 'connections'], status: ArticleStatus.PUBLISHED, aiGenerated: true, viewCount: 234, helpfulCount: 89 },
  ]);
  console.log(`Created ${articles.length} knowledge articles`);

  // === CMDB ===
  const ciRepo = AppDataSource.getRepository(ConfigurationItem);
  const cis = await ciRepo.save([
  ...
  console.log(`Created ${cis.length} CIs`);

  // === WORKFLOWS ===
  const wfRepo = AppDataSource.getRepository(Workflow);
  const workflows = await wfRepo.save([
    ...
    { name: 'SLA Warning Notification', description: 'Send notification when SLA is at risk (30 min before breach)', status: WorkflowStatus.ACTIVE, trigger: { type: WorkflowTriggerType.SLA_BREACH_IMMINENT }, conditions: [], actions: [{ type: 'send_notification', order: 1, config: { type: 'sla_warning' } }, { type: 'send_email', order: 2, config: { template: 'sla_warning' } }], executionCount: 87, successCount: 87 },
  ...
  const catalog = await catalogRepo.save([
    ...
    { name: 'VPN Access Setup', description: 'Set up VPN access for remote work. Includes FortiClient installation and credentials.', category: 'Network', icon: 'Shield', slaHours: 2, requiresApproval: false, formFields: [{ id: '1', label: 'Device Type', type: 'select', required: true, options: ['Windows', 'Mac', 'Linux', 'iOS', 'Android'] }], sortOrder: 4, requestCount: 89 },
    ...
    { name: 'Hardware Repair/Replacement', description: 'Request repair or replacement for faulty hardware including laptops, monitors, keyboards, and peripherals.', category: 'Hardware', icon: 'Wrench', slaHours: 24, requiresApproval: false, formFields: [{ id: '1', label: 'Hardware Type', type: 'select', required: true, options: ['Laptop', 'Monitor', 'Keyboard/Mouse', 'Headset', 'Docking Station', 'Other'] }, { id: '2', label: 'Asset Tag', type: 'text', required: false }, { id: '3', label: 'Issue Description', type: 'textarea', required: true }], sortOrder: 6, requestCount: 67 },
    { name: 'Meeting Room Tech Support', description: 'Get help with conference room AV equipment, video conferencing, and presentation setup.', category: 'Facilities', icon: 'Monitor', slaHours: 1, requiresApproval: false, formFields: [{ id: '1', label: 'Room Number', type: 'text', required: true }, { id: '2', label: 'Meeting Time', type: 'date', required: true }], sortOrder: 7, requestCount: 124 },
  ...
  console.log('\nDemo credentials:');
  console.log('  Admin:  admin@simplenow.io / admin123');
  console.log('  Agent:  sarah.agent@simplenow.io / admin123');
  console.log('  Viewer: viewer@simplenow.io / admin123');
}
